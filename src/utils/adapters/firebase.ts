...
const Adapter = (config: IAdapterConfig, _options = {}) => {
  async function getAdapter(appOptions: AppOptions) {
    ...
    async function createUser(profile: IProfile): Promise<IUser | null> {
      ...
      try {
        const newUserRef = await firestoreAdmin.collection(usersCollection).add({
          name: profile.name,
          email: profile.email,
          image: profile.image,
    ...
    async function getUser(id: IUser['id']): Promise<IUser> {
      ...
      try {
        const snapshot = await firestoreAdmin.collection(usersCollection).doc(id).get()
    ...
    async function getUserByProviderAccountId(
      ...
      try {
        ...
        const userSnapshot = await firestoreAdmin.collection(usersCollection).doc(userId).get()
    ...
    async function updateUser(user: IUser): Promise<IUser> {
      ...
      try {
        await firestoreAdmin.collection(usersCollection).doc(user.id).update(updatedUser)
    ...
    async function linkAccount(
      ...
      try {
        // create Account
        const accountRef = await firestoreAdmin.collection(accountsCollection).add(newAccountData)
    ...
    async function unlinkAccount(
      ...
      try {
        ...
        await firestoreAdmin.collection(accountsCollection).doc(accountId).delete()
      } catch (error) {
    ...
    async function createSession(user: IUser): Promise<ISession> {
      ...
      try {
        const newSessionRef = await firestoreAdmin.collection(sessionsCollection).add({
          userId: user.id,
          expires: expires,
    ...
    async function getSession(sessionToken: ISession['sessionToken']): Promise<ISession | null> {
      ...
      try {
        ...
        if (!snapshot.empty && session.expires && new Date() > session.expires) {
          // delete the session
          await firestoreAdmin.collection(sessionsCollection).doc(snapshot.docs[0].id).delete()
        }
    ...
    async function updateSession(session: ISession, force: boolean): Promise<ISession | null> {
      ...
      try {
        const shouldUpdate =
          SESSION_MAX_AGE && (SESSION_UPDATE_AGE || SESSION_UPDATE_AGE === 0) && session.expires
    ...
    async function deleteSession(sessionToken: ISession['sessionToken']): Promise<void> {
      ...
      try {
        ...
        await firestoreAdmin.collection(sessionsCollection).doc(sessionId).delete()
      } catch (error) {
